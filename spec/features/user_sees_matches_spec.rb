...
feature 'User sees corresponding matches' do
  scenario 'in their own profile page' do
    ...
    user5 = create(:user, name: "Nome5", email: "email5@test.com")

    wish1 = create(:wish, product: 'Pregos', user: user1)
    material1 = create(:wish, product: 'Pregos', user: user2)

    wish2 = create(:wish, product: 'Tinta preta', user: user1)
    material2 = create(:wish, product: 'Tinta preta', user: user3)

    wish3 = create(:wish, product: 'Ripas de madeira', user: user1)

    material3 = create(:wish, product: 'Ferragens', user: user4)
    ...
    within('.matches') do
      expect(page).to     have_content(wish1.product, count: 2)
      expect(page).to     have_content(wish2.product, count: 2)
